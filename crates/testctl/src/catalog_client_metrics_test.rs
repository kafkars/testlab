...
#[test]
fn compatible_kafkars_packs_retain_client_metrics_resource_listing() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let repository = Repository::open(&root)
        .unwrap_or_else(|error| panic!("failed to open test repository: {error}"));
    for path in [
        ...
        assert!(
            pack.scenarios
                .iter()
                .any(|candidate| candidate.ends_with("admin-list-client-metrics-resources.toml")),
            "{path} omitted client-metrics resource listing"
        );
    }
}

...
pub const EVIDENCE_SCHEMA_VERSION: u16 = 141;
...
pub enum BrokerStateObservation {
    ...
    Features(crate::BrokerFeaturesState),
    /// One partition's active producers independently read through Kafka's pinned CLI.
    Producers(crate::BrokerProducersState),
    /// Broker log directories independently read through Kafka's pinned CLI.
    LogDirs(crate::BrokerLogDirsState),
    /// Metadata-quorum state independently joined from Kafka's pinned CLI views.
    MetadataQuorum(crate::BrokerMetadataQuorumState),
    /// One cluster-wide transaction listing independently read through Kafka's pinned CLI.
    ...
    /// Exact post-election leaders independently read from Kafka metadata.
    LeaderElection(crate::BrokerLeaderElectionState),
    ...
    ShareGroupOffset(crate::BrokerShareGroupOffset),

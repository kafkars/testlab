...
pub(crate) fn verify_lifecycle(
    ...
    for step in &scenario.steps {
        ...
        verify_action(&step.action, index, violations);
    }
    verify_finish(index, violations);
}

fn verify_action(action: &ScenarioAction, index: &HistoryIndex, violations: &mut Vec<Violation>) {
    if verify_transaction_lifecycle(action, index, violations) {
        return;
    }
    match action {

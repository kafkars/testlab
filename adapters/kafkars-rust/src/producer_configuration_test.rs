...
fn portable_configuration_selects_every_public_policy_value() {
    for method in [
        ...
        ProducerConfigurationMethod::IndividualSetters,
    ] {
...
fn assert_policy(selected: crate::kafkars_api::ProducerConfig, compression: Compression) {
    assert_eq!(selected.delivery_timeout(), Duration::from_secs(20));
    assert_eq!(selected.compression(), compression);
